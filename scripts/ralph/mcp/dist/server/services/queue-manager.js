...
import { QueueSchema, recordQueueDepth, } from '../../shared/index.js';
 ...
 */
export async function addQueueEntry(options) {
    ...
    // Check for duplicate
    const existing = queue.entries.find(e => e.prdPath === prdPath && e.projectRoot === projectRoot && e.status === 'pending');
    if (existing) {
        return {
            success: false,
 ...
 */
export async function listQueueEntries(options = {}) {
 ...
 */
export async function removeQueueEntry(entryId) {
    const queue = await readQueue();
 ...
 */
export async function updateQueueEntryStatus(entryId, status, claimedBy) {
    const queue = await readQueue();
 ...
 */
export async function clearCompletedEntries() {
    const queue = await readQueue();
    const originalCount = queue.entries.length;
    queue.entries = queue.entries.filter(e => e.status !== 'completed' && e.status !== 'failed');
    await writeQueue(queue);
