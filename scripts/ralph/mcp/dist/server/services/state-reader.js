...
import { InstanceStatusSchema, PrdSchema, DEAD_THRESHOLD_SECONDS, STALE_LOCK_THRESHOLD_SECONDS, } from '../../shared/index.js';
import { getGlobalPaths, getProjectPaths, dirExists, getProjectName, } from '../../shared/paths.js';
/**
 * Read instance status from a status.json file
 */
export async function readInstanceStatus(instanceDir) {
 ...
 */
export function getInstanceInfo(status, instanceDir) {
 ...
 */
export async function readLocalInstances(instancesDir) {
    const instances = [];
 ...
 */
export async function readGlobalInstances() {
    const instances = [];
    ...
    try {
        const entries = await fs.readdir(globalPaths.globalInstancesDir, { withFileTypes: true });
        for (const entry of entries) {
...
export async function getAllInstances(options) {
    const { projectRoot, includeGlobal = true, includeDead = false, } = options;
    let instances = [];
 ...
 * Read PRD file
 */
export async function readPrd(prdPath) {
    try {
 ...
 */
export async function getPrdProgress(prdPath) {
 ...
 */
export async function readLocks(locksDir) {
    const locks = [];
    const now = Math.floor(Date.now() / 1000);
